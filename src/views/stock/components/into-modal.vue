        ...
        <i-row>
          <i-col span="11">
            ...
            <div class="table-wrapper left-table-wrapper">
              <Table size="small"  border :columns="goodsHeader" :data="allGoods" no-data-text="暂无数据" class="left-table" height="300"></Table>
              <Page size="small" :current.sync="currentPage" :page-size='8' :total="leftPageTotal" show-elevator @on-change="leftChangePage" class="left-page"></Page>
            </div>
          </i-col>
          <i-col span="12" offset="1">
            <div  class="title-name">入库商品</div>
            <div class="table-wrapper">
            <Table size="small"  border :columns="RKGoodsHeader" :data="RKGoods" height="300" no-data-text="暂无数据"></Table>
            </div>
          </i-col>
        </i-row>
      </Form>
				...
				{title: '库存', key: 'stock'},
				{
					...
					render: (h, params) => {
						...
						return h('Button', {
							...
							on: {
								click: () => {
									for (let i = 0; i < that.RKGoods.length; i++) {
										if (that.RKGoods[i].goodName === params.row.goodName) {
											return;
										}
									}
									if (that.RKGoods.length < 8) {
										let temp = {};
										...
										temp.comment = '';
										that.RKGoods.push(temp);
										that.$emit('changeModalList', that.RKGoods);
									}
								}
							}
						}, '添加');
				...
				{ title: '总价', key: 'totalPrice' },
				{
					...
					render: (h, params) => {
						...
						return h(QlInputNumber, {
							props: {
								value: that.RKGoods[params.index].tempNumber
							},
							on: {
								input (number) {
									if (number === 0) {
										that.RKGoods.splice(params.index, 1);
									} else {
										that.RKGoods[params.index].tempNumber = number;
										that.RKGoods[params.index].totalPrice = (that.RKGoods[params.index].unitPrice * 1000 * number) / 1000;
										that.$emit('changeModalList', that.RKGoods);
									}
								}
							}
				...
				},
				{
					...
					render: (h, params) => {
						...
						return h('Input', {
							...
							on: {
								'on-blur': (e) => {
									vm.RKGoods[params.index].comment = e.target.value;
									vm.$emit('changeModalList', vm.RKGoods);
								}
							}
						});
					}
